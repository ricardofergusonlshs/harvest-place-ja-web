  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#FAF8F0_0%,#F4F9F2_52%,#FFFEFC_100%)] px-4 py-8 text-[#1E2A21] sm:px-6 lg:px-10">
      <section className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        ...
        <Card className="rounded-[34px] border border-[#D8E5D4] bg-white p-6 shadow-[0_24px_80px_rgba(24,59,40,0.12)] sm:p-8">
          <Badge tone="green">
          ...
          <h1 className="mt-4 text-4xl font-black tracking-[-0.05em] text-[#183B28]">
            {isLogin ? 'Sign in to continue' : isSignup ? 'Create your account' : 'Reset your password'}
          </h1>

          <p className="mt-3 text-sm font-semibold leading-7 text-[#5F6A62]">
            {isLogin
              ? 'Access your box, checkout, orders, rewards, and account dashboard.'
              : isSignup
                ? 'Join the marketplace and start shopping fresh Jamaican produce.'
    ...
    <section className="relative overflow-hidden rounded-[34px] bg-[#183B28] px-6 py-8 text-white shadow-[0_30px_90px_rgba(24,59,40,0.20)] sm:px-8 lg:px-10">
      ...
      <div className="relative z-10">
        ...
        <h2 className="mt-5 max-w-xl text-4xl font-black leading-[0.96] tracking-[-0.055em] sm:text-5xl">
          Fresh market access, protected by Supabase Auth.
        </h2>

        <p className="mt-4 max-w-lg text-sm font-semibold leading-7 text-white/78 sm:text-base">
          Sign in to manage your box, track orders, save profile details, view farmer tools, and keep your marketplace activity secure.
        </p>

        <div className="mt-7 grid gap-3">
          <HeroLine icon={<ShieldCheck className="h-5 w-5" />} text="Secure account sessions" />
          <HeroLine icon={<ShoppingIcon />} text="Saved cart and checkout access" />
          <HeroLine icon={<Sparkles className="h-5 w-5" />} text="Rewards, orders, and subscriptions" />
        </div>
      </div>
    </section>
  );
...
function HeroLine({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
